...
class UserDataMatcher:
    ...
    def _is_unit_name_field(self, source_field: str, target_field: str) -> bool:
        """检查是否为单位名称字段"""
        unit_name_keywords = [
            '单位名称', '公司名称', '企业名称', '机构名称', '组织名称',
            'unit_name', 'company_name', 'enterprise_name', 'org_name',
            'dwmc', 'gsmc', 'qymc', 'jgmc', 'zzmc', 'name', '名称'
        ]
        ...
        # 检查字段名是否包含单位名称关键词
        for keyword in unit_name_keywords:
    ...
    def _calculate_field_similarity(self, value1: str, value2: str, matcher, source_field: str = '', target_field: str = '') -> float:
        ...
        """
        try:
            # 【新增】优先检查是否为单位名称字段，使用智能单位名称匹配
            if self._is_unit_name_field(source_field, target_field):
                similarity = self.intelligent_unit_matcher.calculate_similarity(value1, value2)
                logger.debug(f"智能单位名称匹配: {source_field}->{target_field}, {value1[:20]}... <-> {value2[:20]}... = {similarity:.3f}")
                return similarity
            
            # 【关键修复】检查是否为地址字段，使用地址语义匹配
            elif self._is_address_field_simple(source_field, target_field):
