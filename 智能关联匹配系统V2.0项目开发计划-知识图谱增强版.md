...
### 🚀 第一阶段：核心架构升级（4-5周）**[扩展1周]**

#### 第1周：数据管理基础设施 + 知识图谱基础
**目标**：建立CSV数据源管理和**知识图谱基础架构**

**主要任务**：
1. **CSV数据管理模块** (1-2天)
   ```python
   # 新增模块：src/data_manager/
   ├── csv_processor.py      # CSV文件处理器
   ├── data_analyzer.py      # 数据结构分析器
   ├── schema_detector.py    # 字段类型检测器
   ├── kg_data_adapter.py    # 知识图谱数据适配器 ⭐ **新增**
   └── validation_engine.py  # 数据验证引擎
   ```

2. **知识图谱基础架构** (2-3天) ⭐ **重要新增**
   ```python
   # 新增模块：src/knowledge_graph/
   ├── __init__.py
   ├── kg_models.py          # 知识图谱数据模型
